  ...
  for(i = 0; i < height; i++)
  {
    ...
    else if(DrawProp[DrawProp->LcdLayer].LcdPixelFormat == LCD_PIXEL_FORMAT_ARGB4444)
    {
      for (j = 0; j < width; j++)
      {
          ...
          if(line & (1 << (width- j + offset- 1)))
          {
            argb4444[width-j-1] = CONVERTARGB88882ARGB4444(DrawProp[DrawProp->LcdLayer].TextColor);
          }
          else
          {
            argb4444[width-j-1] = CONVERTARGB88882ARGB4444(DrawProp[DrawProp->LcdLayer].BackColor);
          }
#else
          if(line & (1 << (width- j + offset- 1)))
          {
