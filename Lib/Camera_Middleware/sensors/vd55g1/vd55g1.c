...
#define VD55G1_REG_PWL_CTRL                           0x032b
#define VD55G1_REG_PWL_LUTSEL                         0x052c
#define VD55G1_REG_DARKCAL_PEDESTAL                   0x0526
...
static int VD55G1_SetupMirrorFlip(VD55G1_Ctx_t *ctx)
{
  ...
  /* turn on PWL */                                 
  ret = ctx->write8(ctx, VD55G1_REG_PWL_CTRL, 0x01); // PWL_CTRL = Enable
  ret = ctx->write8(ctx, VD55G1_REG_PWL_LUTSEL, 0x00); // PWL_LUTSEL = [0, 1]
  ret = ctx->write16(ctx, VD55G1_REG_DARKCAL_PEDESTAL, 8); // DARKCAL_PEDESTAL = 16
...
int VD55G1_Start(VD55G1_Ctx_t *ctx)
{
  ...
  VD55G1_SetBrightnessLevel(ctx, 0x1E);
